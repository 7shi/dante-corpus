...
on record S3.11 in [`stages/03.md`](03.md)). Scope closed as **full-corpus
...
**The suite's running composition, kept here** (moved out of
[`../PLAN.md`](../PLAN.md) on 2026-09-05, which now carries the current count
and this pointer only — this note was already the place its history lived).
Counts are cumulative repo-wide, harness and corpus tests in one run:

- **1,001** — the count at Stage 8's close: 999 + 2 from S8.3's per-row salvage
  scope.
- **999** — 991 + 8 from S8.1's level-2 class, its gate-admissibility guard
  (generalized to the whole level table on 2026-09-05, not re-counted), and the
  two end-to-end `--fix 2` runs.
- **991** — 987 + 4 from the repo-wide Markdown link sweep,
  `tests/test_docs_links.py`, added 2026-09-03 after `ee47f34` repaired 23
  broken cross-document links; corpus-side, not harness-side.
- **987** — 876 + 11 (S5.1) + 8 (S5.2) + 21 (S5.3) + 9 (S5.5) + 9 (S5.7) +
  4 (S5.8) + 17 (S6.2) + 2 (around the llm7shi 0.15.0 status-bar rework) +
  3 (S6.4) + 9 (S6.6) + 2 (S6.8) + 4 (S6.10) + 12 (S7.1). S6.3, S6.5, S6.7,
  S6.9 and S6.11 added none, being corpus runs.
- **858 / 857 / 864** — this stage's own arithmetic, above: the `TokenBucket`
  removal took 864 → 857, and the mid-canto kill regression test took it to 858.
...
- **S4.3 — 2026-08-29: corpus-wide readout tool + results; STAGE 4 CLOSED on
  ...
  **Results**: hygiene all-clear on every check (zero token-assertion
  ...
  confirmation runs (S3.4/S3.9/S3.10/S3.11 in [`stages/03.md`](03.md)) —
