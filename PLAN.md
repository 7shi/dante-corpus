...
> **Phase 7 is open, all structural outliers are closed, and divergence residue is 104.** Layer 5 stands
> at **0 hard / 104 soft** (all 104 standard argument divergence positions; 0 `dual_role`, 0 `extra_tuple`,
...
> Round 10 log audits & driver fix is **§P9**, and the assistant-side read census across extra_arg is **§P10**.
>
> **Phase 7 is: drive soft to 0, and when a fix fails, find out why.** The assistant-side read census
> resolved one Layer-4 retag and dropped 6 spurious argument positions (**112 → 104**), Round 10 log audits
> resolved one Layer-4 retag and dropped 3 spurious rows with driver fix (**116 → 112**), the tenth round went
...
> the seven structural outlier positions were resolved (**126 → 119**, inferno 30, purgatorio 35, paradiso 39).
>
...
>
> **Layer 5 is operating under Phase 7 with 0 hard / 104 soft violations** (all 104 standard argument
> divergence positions; all structural outliers and artifact-internal contradictions 0). Per canticle:
> inferno 30, purgatorio 35, paradiso 39. Checks: `dep --check` **0 hard / 0 soft**, `case --check` 0 hard,
> `skel --check` 0 hard / **104** soft, `np --check` 0/0, `morph --check` 0/0, `pytest` **544 passed**. The
...
- **Latest Improvements** (the full chronology is in [`skel/PHASE6.md`](skel/PHASE6.md) §3):
  - **Assistant-side read census on `extra_arg` (2026-08-18)**: **112 → 104 (−8, −7.1%)**; `pytest` **544**;
...
- **Layer 5 — skeleton**: implemented, all 100 cantos built, checker refined through Phases 0-5r
  ...
  three canticles reports **0 hard, 104 soft** (down from 17438 at the first full-corpus
