...
> **The Paradiso read series is two batches in.** All of Inferno, all of Purgatorio and now
> **Paradiso 1–10** have been read per-position: rules DK–DR took **288 → 261 (−27, −9.4%)** with
> zero model calls, 0 hard, `pytest` **465 passed**, plus 10 Layer-4 rows, 2 Layer-2 rows and
> 2 case-annex rows. Per canticle: inferno 56, purgatorio 84, paradiso 121.
>
> **The plan for the rest of the work is unchanged:**
>
> > **Read Paradiso in 5-canto batches — 11–15, 16–20, 21–25, 26–30, 31–33 — and when the
> > series is finished, run one `--fix` round.** No round in between.
>
> **The next session's task: the per-position read of Paradiso 11–15 — 28 soft violations at base
> 261.** Start a fresh session, list them with `uv run skel.py paradiso --check -c <n>` from
> `skel/`, and read each one with `uv run read.py paradiso <canto> <line>`. Five batches, 99
> positions, schedule **re-measured at base 261** in [`skel/PLAN.md`](skel/PLAN.md)'s *The Read
> Series*. **Re-measure the batch first**; every landed rule shrinks the batches after it, and
> rules DK–DR moved all three canticles.
>
> **What the Paradiso 6–10 batch found.** Its transferable lesson is about a rule's own prose: **a
> rule's docstring can be more correct than its code**. Rules DB and AK each state the gate that
> decides them (*the copula must have no other complement*; *a token no layer calls a
> preposition*) and each carried a second, narrower condition — a part-of-speech test — inherited
> unexamined from the one line that motivated it. Dropping those two conditions (rules DL, DM)
> took 6 positions across five cantos and all three canticles. This is the Purgatorio 26–30
> batch's finding with the polarity reversed, and the pair is worth holding together: **prose and
> gate disagreeing is a finding either way; census decides which to keep.**
>
> Second, **a scope refusal is about the instrument, not the shape** — rule DQ reaches five
> positions of the impersonal-verb family the Paradiso 1–5 batch dropped at census 29 as needing
> a verb-valency lexicon, by asking a structural question (is the subject inherited, and is a
> `ccomp` the predicate's only other derived argument?) instead of a lexical one. Third, **check a
> rule's test against both ends of the relation it is about**: rule AG compares the inherited
> nominal with the recipient predicate and never the two *predicates* with each other (rule DO,
> 30 of 1151 candidates). Fourth, **price a derivation rule as an acceptance before writing it**
> — rule DN in `derive_unit` measured −4/**+40** and on the acceptance side −1/+0, the second
> instance after rule CS's variant (+180).
>
> **The sixth round's one candidate is still unwritten**: the convention clause for the
> prepositional adjunct of time or place, against `missing_arg obl*` at **57 of 261** when last
> measured (bare `obl` 18), still the residue's largest single bucket. **Write it during the
> series**, then let the round decide it. **Do not run a round while checker rules are being
> written against the base it would move, and do not run one with nothing on the scale.**
>
...
**Layer 5 is operating under Phase 6 with 0 hard / 261 soft violations.**
...
read, 6 more the same day by the Paradiso 1–5 read, 10 more by the Paradiso 6–10 read),
...
by the Paradiso 6–10 read), `skel --check` 0 hard/**261** soft (inferno 56, purgatorio 84,
paradiso 121),
`np --check` 0/0 (1 span split, 1 widened, 1 added, 1 moved 2026-08-16, 1 clitic span added
2026-08-17), `morph --check` 0/0
(3 + 5 + 1 + 2 + 8 + 2 + 1 + 1 + 1 + 2 rows corrected), `pytest` **465 passed** (the Purgatorio
26–30 read added 10, the Purgatorio 31–33 read 4 more, the Paradiso 1–5 read 8 more, the Paradiso
6–10 read 16 more). The fifth `--fix` round (2026-08-17) touched
...
- **The plan for the rest of the work (decided 2026-08-17)**: **read Paradiso in 5-canto batches — 11–15, 16–20, 21–25, 26–30, 31–33 — and run one `--fix` round when the series is finished.** No round in between. Paradiso 1–5 (rules DG–DJ) and 6–10 (rules DK–DR) are read.
- **The read series is the standing task**: per-position reads of all 100 cantos in 5-canto batches. **Inferno, Purgatorio and Paradiso 1–10 are complete**; **Paradiso 11–15 is next** (28 soft at base 261), running to Paradiso 33. Five batches, 99 positions, all of them Paradiso; schedule (re-measured at base 261), reasoning and the eight-step per-batch procedure in [`skel/PLAN.md`](skel/PLAN.md)'s *The Read Series* / *How to Read a Batch*. Tool: `skel/read.py`.
- **The sixth `--fix` round has one candidate queued, and it is unwritten** (`make -C skel fix`, run 3-way parallel — the user's to run). The fourth round emptied the prompt queue on 2026-08-16 and the fifth (2026-08-17) ran with nothing on it; the Purgatorio 16–20 read's one contribution — a convention clause for the **prepositional adjunct of time or place**, against `missing_arg obl*` now at **62 of 298**, still the residue's largest single bucket — has never been written into `skel/skel.py`. **Write it during the series**, then let the round decide it. See [`skel/PLAN.md`](skel/PLAN.md)'s *The Sixth `--fix` Round*.
- **Inferno's 56, Purgatorio's 84 and Paradiso 1–10's 22 standing positions** are the read batches' residue with two `--fix` rounds now over them — the most direct sample of what a round leaves behind. Not urgent: the five unread batches come first.
- **Other Assistant-Side Tasks** (populations at base 541 unless noted, so a re-measure at 261 comes first; folded into the batch that covers them):
  ...
  - **Read `missing_arg obl*` (re-measured at **57** of 261, bare `obl` 18; still the residue's largest single bucket).** The Purgatorio 16–20 read found four of them to be plain omissions of an adjunct or a dative the tree records, which is also the sixth round's one prompt candidate, still unwritten.
...
checker was refined through Phases 0-5r, rules V through DR, and Phase 6's restructuring plus five `--fix` rounds, bringing its soft residue to **261**
...
**The open route is the read series, then one final `--fix` round**: per-position reads of all 100 cantos in 5-canto batches. Inferno, Purgatorio and Paradiso 1–10 are complete; Paradiso 11–15 is next, running to Paradiso 33, and the sixth round follows the last batch (see *Next Steps & Open Routes* above and [`skel/PLAN.md`](skel/PLAN.md)). All five layers plus the case extension are implemented, built for all 100
...
- **Layer 5 — skeleton**: implemented, all 100 cantos built, checker refined through Phases 0-5r
  ...
  further shapes where the derivation was silent rather than disagreeing, and the AG-DJ series
  from the Inferno 4-6, 7-10, 11-15, 16-20, 21-25, 26-30 and 31-34, the Purgatorio 1-5, 6-10,
  11-15, 16-20, 21-25, 26-30 and 31-33 and the Paradiso 1-5 and 6-10 per-position reads; see
  ...
  three canticles reports **0 hard, 261 soft** (down from 17438 at the first full-corpus
...
4. **Layer 5 (skeleton)** — *implemented* (`dante_corpus/skel.py` + `dante_corpus/hashes.py` +
   ...
   CP-CT series, the CU-CY series, the CZ-DD series, the DE-DF series, the DG-DJ series and the DK-DR series, with
   `--fix` restructured in Phase 6 and five rounds run (`--check`: 0 hard / 261 soft). Phase 5 closed with every route measured; see
