...
refined through Phases 0-5j — see *The layers* below and [`skel/README.md`](skel/README.md) for
...
- **Layer 5 — skeleton**: implemented, all 100 cantos built, checker refined through four
  ...
  three canticles reports **0 hard, 3924 soft** (down from 17438 at the first full-corpus
  ...
  5a-5j have landed (rules C/D/L/M/N/O, two re-triage rounds, one full `--fix` pass, and the first
...
1. **Layer 5 Phase 5** — eliminate the residual soft violations, tracked in
   [`skel/PLAN.md`](skel/PLAN.md) (*Next session — start here*). Corpus is at **0 hard, 3924
   soft**. The queued items are the `xcomp`/`ccomp`/`obj` labeling cluster, then a re-triage of
   `extra_arg` (1887) and `missing_arg` (1239), which together are 77% of what remains. The
...
4. **Layer 5 (skeleton)** — *implemented* (`dante_corpus/skel.py` + `dante_corpus/hashes.py` +
   `skel/skel.py`), all 100 cantos built, checker refined through Phases 0-5j
   (`--check`: 0 hard / 3924 soft). Driving that residual to zero is the remaining work, tracked
