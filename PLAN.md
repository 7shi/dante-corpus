...
### Handoff (2026-09-05) — resume here

> **Current State & Baseline**:
> - **All five layers & pronoun case annex**: **0 hard / 0 soft violations across all 100 cantos** (Inferno 0, Purgatorio 0, Paradiso 0; `pytest` **1,001 passed** — 552 corpus + 449 harness).
...
>
> **Active work: Dedicated Grammar Agent Harness (`harness/`)**: Stages 1–8
...
> reduction, refactoring, soft `--fix` level 2) are complete; **Stage 9 is
> open** (opened 2026-09-05), scoped to fixed-context execution — replacing the
> per-unit tool-calling session with a fixed-length context and a loop, whose
> per-iteration improvement signal is the stage's first problem. The harness's
> own reconstruction of Layer 5 is hard-clean (0 hard / **3,138** soft against
> the derivation contract, down from 4,624 over Stage 8's two corpus-wide fix
> runs) and does not touch gold `skel/`.
...
- **Test Suite**: `pytest` **1,001 passed** (552 corpus + 449 harness; corpus tests ~1 s).
...
1. **Dedicated Grammar Agent Harness for Local LLMs (`harness/`)**:
   - A specialized agent harness in `harness/` adopting a staged bottom-up
     ...
     `--fix` level 2, all closed; Stage 9 (fixed-context execution) is open.
...
at `pytest` 1,001 passed including the harness).
