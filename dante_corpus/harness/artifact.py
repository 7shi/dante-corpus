"""The run's durable artifact: a canto's TSV, and how it is rendered.

Split out of `reconstruct.py` (S7.2). Two things live here, and they are one
thing: the renderer that must stay byte-exact with the subject's own writer
(`skel.io.write_skel` for Layer 5), and the file that is written through it unit
by unit and read back to resume (`../../harness/stages/05.md` record S5.5).
Keeping them together is what keeps the streamed file byte-identical to a
single-pass `render_tsv` — the property `TsvArtifact` depends on and
`test_render_tsv_matches_write_skel_bytes` pins.

Every fact about a row — its type, its order, its columns — arrives as a
`RowCodec`, so nothing here knows what a row means. Nor does anything here know
about gates, sessions, or gold.
"""
...
from .rows import Row, RowCodec

__all__ = ["TsvArtifact", "render_tsv"]


def render_tsv(lines: list[tuple[int, list[Row]]], *, codec: RowCodec) -> str:
    ...
    """
    return "\t".join(codec.header) + "\n" + _render_body(lines, codec)


def _render_body(lines: list[tuple[int, list[Row]]], codec: RowCodec) -> str:
    ...
    for no, rows in lines:
        if not rows:
            out.append("\t".join(codec.sentinel_cells(no)))
            continue
        for row in codec.sorted(rows):
            out.append("\t".join(codec.cells(no, row)))
    return "\n".join(out) + "\n"


class TsvArtifact:
    """A canto's TSV, written unit by unit and read back to resume.
    ...
    *partially* deleted unit counts as unsettled too, and its surviving rows
    ...
    def __init__(self, path: str | Path, *, codec: RowCodec):
        self.path = Path(path)
        self.codec = codec
    ...
    def load(self) -> dict[int, list[Row]]:
        """Parse the artifact on disk (missing file = nothing settled yet).

        Mirror of the subject's own loader over an arbitrary path: a sentinel
        row registers its line as present without contributing a row, which is
        exactly what the settled-unit test needs. Which row that is, and how the
        cells become one, is the codec's business.
        """
        self.rows = {}
        ...
        for index, text in enumerate(
            ...
            if index == 0 or not text:  # header / blank
                continue
            no, row = self.codec.parse_cells(text.split("\t"))
            bucket = self.rows.setdefault(no, [])
            if row is not None:  # a sentinel line registers, contributes nothing
                bucket.append(row)
        return self.rows

    def settled(
        self, units: list[list[int]]
    ) -> dict[tuple[int, int], dict[int, list[Row]]]:
        ...
        """
        result: dict[tuple[int, int], dict[int, list[Row]]] = {}
        for group in units:
    ...
    def write_unit(self, group: list[int], rows: dict[int, list[Row]]) -> None:
        ...
        if self._append_only:
            ...
            with open(self.path, "a", encoding="utf-8") as fh:
                if new:
                    fh.write("\t".join(self.codec.header) + "\n")
                fh.write(
                    _render_body([(no, self.rows[no]) for no in group], self.codec)
                )
    ...
    def rewrite(self) -> None:
        ...
        payload = render_tsv(
            [(no, self.rows[no]) for no in sorted(self.rows)], codec=self.codec
        )
