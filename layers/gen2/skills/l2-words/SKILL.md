...
- **A participle is `verb`**, always — past or present, and whether it stands in a compound tense, predicatively, or in front of a noun like an adjective. **Here is why, because the reason is what makes this answerable rather than arbitrary:** being a participle is not a category, it is a form the verb takes, and this pass writes categories only — the later pass that reads tense and mood off these words is where *participle* gets written down. That pass can add the form to a `verb`. It cannot recover the verb from an `adjective`, because nothing in the row would say a verb had ever been there. So `adjective` at a participle does not record a finer judgment, it destroys a coarse one; `verb` keeps both halves and costs nothing. The rule holds hardest where the participle reads most like an adjective — attributive, before or after its noun, with nothing verbal around it. Those are the positions where the distinction is most worth keeping, so they are the ones to be most careful about, not the exceptions.
