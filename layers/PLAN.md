...
**L1/L2 design (`REDESIGN.md` §2.1–§2.2, §5).** Premise 3 as above. **L1** =
`tokenize()`'s full output, punctuation included, own numbering — design,
measurement, and implementation: [`L1.md`](L1.md). **L2** = grammatical words
as `(l1_index, text)` pairs, split → POS-classify → normalize, only splits and
never merges (the `fixed` many:1 direction stays a relation one layer up, over
L2 entries) — full design, cost figures, and rollout plan: [`L2.md`](L2.md),
not repeated here. **Build order**: L4 (dependency) precedes L3 (phrases) — L3
must project from generation 2's own L4, never from old Layer 4.

---

## L2, built and closed out over *Inferno* 1 (2026-09-09 – 2026-09-10)

This section replaces the session-by-session handoff trail that built L2 —
the full argument for every decision below lives in [`L2.md`](L2.md), which is
where it is cited from; only the decisions and the numbers are kept here.

- **L1 implemented** ([`gen2/l1.py`](gen2/l1.py)), verified against §2.1 in
  full. Tests: [`tests/test_gen2_l1.py`](tests/test_gen2_l1.py).
- **L2 step 1 (split only) implemented and tried** — first over *Inf* 1:1-9
  (3 requests, 0 refusals, `pel` untouched), then over the full canto (43
  chunks). The **checker was reworked three times** to fix a wordform-pooling
  artifact (`ours`/`ours-ambiguous` renamed `L2`/`l2-ambiguous`, a per-line
  `[...]` render added, `precedent-ambiguous` dropped). Final read: **503
  agrees, 8 differs, 0/0**. Of the 8, six are the designed surface/lemma gap
  (step 3 not run); two were decided by corpus-wide precedent, never gold:
  **`pel`** (1:33) — L2 wrong, oversplit `per`+`il`, `pelo` (fur) in all 7
  corpus occurrences; **`'ncontro`** (1:59) — old Layer 2 wrong, inconsistent
  with itself corpus-wide.
- **A three-run check** (same model/prompt, three draws) established the
  standing lesson: **repeated runs detect disagreement, they do not verify
  correctness.** `pel` reproduced its wrong answer all three times
  (systematic); `dipartilla` split 2:1 toward the *wrong* reading, corrected
  by old Layer 2's `remote past` precedent (majority can be wrong too).
- **Hand-editing the artifact was adopted as policy** (steering a prompt to a
  known answer and correcting it afterward are the same human judgment by two
  routes; only the second leaves a record) and **`l2/CORRECTIONS.md`**
  created, matching the four other layers' `CORRECTIONS.md` precedent.
- **Step 0 (restore dropped letters before the split) was built, run, and its
  hypothesis rejected**: it worked as normalization (136/993 tokens restored
  correctly) but did not catch `pel` — the trap needs a contentful column at
  the *same* position, not a separate pre-pass. Cost was 6.2× the split-only
  pass for no gain at the one job it was built for.
- **Split and POS were merged into one pass** (the position old Layer 2 itself
  held: the split and its grammatical justification answered together, so an
  unwritable POS sequence blocks a wrong split at the moment it would be
  made). Lemmas are excluded; restorations are folded in, retiring
  `gen2/restore.py`'s job (module kept, unused). Tags are a closed set of ten;
  a participle is `verb` by convention. An `Index` column was added so a
  refusal can name an addressable row. All four prior split-only artifacts
  (`01.tsv`, `01-2/3/4.tsv`, `01-restore.tsv`) were deleted — what they
  measured survives in `L2.md`, not as files.
- **The merged pass ran live, twice, then a third time this session.** Run 1:
  `pel` and `dipartilla` both correct, 8 differs → 4, 59.5 min. Run 2 (after
  `Index`): both reproduced correct, **953/989 tags agree (96.4%)**, but the
  `[pos differs]` set itself moved 41→36 between runs — evidence that *tag*
  wobble, not just split wobble, is real. Run 3 (this session, under a prompt
  that states the participle convention's *reason* rather than a bare rule):
  **`disperate`/`dolenti` came back `verb`, confirming the reason reached the
  model** — but **`pel` and `dipartilla` both flipped back to their wrong
  answers**, which reframes the two "reproduces" results from runs 1–2:
  **two agreements were two draws, not a settled mechanism** — the same
  sampling-variance lesson the three-run check and the tag wobble had already
  shown, now confirmed at the two positions that mattered most (full
  argument: `L2.md`, *A third run*).
- **The eleventh tag (`participle`) was decided against** (old Layer 2's own
  723:31 majority already puts participle-hood in `tense`, not `pos`); the
  convention was instead given its reason in the prompt, which run 3 confirms
  works.
- **`l2/CORRECTIONS.md` rewritten against run 3** (this session): the seven
  entries decided by runs 1–2 (`poco` 1:7, `brame`, `tutte`, `Molti`,
  `imperador`, `disperate`, `dolenti`) are now correct unaided and dropped per
  the file's own rewrite rule; `me'` 1:112 carried forward (still unrestored);
  **`pel` and `dipartilla` added**, argued from the point above rather than
  treated as fresh defects. All three hand-corrected into
  `layers/l2/inferno/01.tsv`. `--check` after: **507 agrees, 4 differs, 0/0**,
  **950/989 tags (96.06%)**, 144 restorations. `uv run pytest layers/tests` →
  **135 passed**.
- **One position was checked and deliberately left uncorrected**: `poco`
  after `un` (1:19, 1:28) reads `noun` against old Layer 2's `adverb`, but old
  Layer 2 itself splits 21/29 adverb/noun on this exact construction
  corpus-wide — unlike 1:7's `poco` (0× `pronoun` anywhere), there is no
  majority precedent to correct toward (`L2.md`, same section).

**Still open, carried forward unchanged:**

- **Widening past *Inferno* 1.** `layer2-ambiguous` and `l2-ambiguous` have
  never fired on real data (0 rows every run) — a wider canto is where both
  either exercise for real or stay unexercised in practice too.
- **The corpus-wide `del`/`nel`-shaped measurement** (how many such tokens are
  not contractions) is still undone, though `pel`'s own status — real but
  intermittent, not a blocked-forever trap — lowers its stakes.
- **The bootstrap safe/contaminated split**, **the external consumer survey**,
  and **§4.1's cross-layer checker design** (`REDESIGN.md` §5) are all
  unrevisited since the first review session.
- **Steps 2/3 of the original three-step L2 design are moot** — the merged
  pass answers split, POS and restoration together; nothing further is
  planned there.
