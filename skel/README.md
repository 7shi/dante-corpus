...
**Status: built for all 100 cantos, checker refined through Phase 5 (5,919 → 2,084 soft) and Phase 6 (rules AG–EH plus seven `--fix` rounds: 160 soft), and now under Phase 7, whose eighth/ninth rounds, rule EI, census reads/retags, systematic failure fixes, dual_role resolution, and outlier position fixes took it to 119 soft — all 119 standard argument divergence positions, with 0 structural outliers or artifact-internal contradictions. `--fix` operates as a three-stage driver (deterministic auto-repair, POS-keyed micro-prompts, and fallback regeneration — the last measured and switched off permanently from round 8 on).**

`make -C skel check`: **0 hard, 119 soft** violations across all 100 cantos (the divergence residue is 119, down from 17,438 at the first full-corpus measurement; all structural outliers and internal contradictions are 0 — see [`CORRECTIONS.md`](CORRECTIONS.md)). Full historical measurement tables, per-phase progressions and empirical findings are documented in [`PHASE5.md`](PHASE5.md) (Phase 5) and [`PHASE6.md`](PHASE6.md) (Phase 6 — the seven rounds, the nineteen-batch read of all 100 cantos, and its transferable findings). For current Phase 7 operating principles, active routes, the failure shapes and outlier write-ups (§P5, §P6, §P7) and driver architecture, see [`PLAN.md`](PLAN.md) and [`CORRECTIONS.md`](CORRECTIONS.md).
...
28. **Phase 6 — rules DS-DW, from the Paradiso 11-20 read** (2026-08-17). **Five** Layer-5 rules,
    each censused, measured alone by violation diff and mutation-checked; together with 9 Layer-4
    rows and 1 Layer-2 row, **261 → 245 soft**, no model calls. Two batches read in one session.
    The batch's finding is about the acceptance test itself: a candidate that measured **−3/+0** —
    the profile every kept rule in the series has — was dropped after *reading the positions it
    removed*, which were two control verbs whose inherited subject was correct. Measure by diff,
    then read the diff. Full evidence in [`CORRECTIONS.md`](CORRECTIONS.md).
    - **DU** (`derive_unit`, step 3): the shared-subject propagation across `conj` **stops at a
      conjunct Layer 4 marks with its own subordinator**, and does not walk through one either.
      UD's shared-subject convention is about coordination; a clause with its own `mark` is
      subordinate, and its subject is its own ("che **averle** dentro e sostener lo puzzo",
      "**onde** la visïon crescer **convene**", "**onde** un poco mi **piace**"). Censused at 49
      `conj` verbs with no subject of their own carrying a `mark`; −8/+1, the +1 a correctness
      trade that exposes an LLM misreading the old tree had matched (purgatorio 21:80).
    - **DW** (`_depictive_attr_omitted`): rule BX's `attr` leg — the depictive Layer 4 wrote in
      the complement slot rather than loose as an `obl` ("e **portera'ne scritto** ne la mente",
      "mi facieno **stimar, veloci e lente**"). Layer 4 picks between its two secondary-predicate
      deprels by position, not by function, so accepting only one of them makes the reading depend
      on the notation. Gated as rule DB is — the predicate must derive some other non-subject
      argument, so a copula's sole complement stays flagged — plus rule BX's own two (no `case`
      child, adjective POS). Censused at 100; −2/+0.
    - **DS** (membership check): rule BW, applied where the citation is still raw. Rule BW's own
      docstring cites paradiso 19:74 ("sappiendo … **quanto** costa") and that position was still
      flagged, because the membership check runs before `_classify_divergence` and rejects a
      `mark` as heading no NP, pronoun or predicate. Rule AQ′'s lesson a third time (after rule
      DG). Same gates as rule BW; censused at 325 non-conjunction `mark` rows; −1/+0.
    - **DT** (`_coordination_head`): `compound` collapses onto its head like `flat` does (rule
      BE) — a multiword name is one nominal, and citing any of its tokens cites it ("una
      Cianghella, un **Lapo** Salterello"). Censused at **2** rows corpus-wide and kept anyway, on
      rule BZ's ground: it is rule BE's reasoning verbatim, and without it the collapse depends on
      which of two interchangeable deprels Layer 4 picked. −1/+0.
    - **DV** (`_stranded_on_underived_complement`): rule CB read through rule AU's host. Rule CB
      accepts an oblique the tree hangs on an `attr`/`xcomp` complement the derivation never
      promotes; rule AU already says that an `amod` adjective over one of the predicate's own
      arguments is its secondary predicate, and the same reasoning applies there ("fu fatta già la
      **terra degna** / **di tutta l'animal perfezïone**"). Censused at 119; −1/+0.

29. **Phase 6 — rules DX-EA, from the Paradiso 21-25 read** (2026-08-17). **Four** Layer-5 rules,
    each censused, measured alone by violation diff and mutation-checked; together with 10 Layer-4
    rows, 1 Layer-2 row and 1 Layer-3 span, **245 → 234 soft**, no model calls. The batch's finding
    is a second way to price a candidate that the acceptance test alone would pass: paradiso 21:5
    wants rule CX's role gate widened to `obl`, and the corpus's *other* instance of the same
    construction (paradiso 23:14, already accepted by rule DJ) shows the `obl` is a Layer-4
    convention applied consistently — so the divergence is a second claim about the role, which is
    what the gate exists to keep out. Full evidence in [`CORRECTIONS.md`](CORRECTIONS.md).
    - **DX** (`_predicative_advmod`): rule R's **noun** leg — the depictive nominal Layer 4 hangs
      `advmod` on the predicate ("ov' io **dormi' agnello**"). Rule R's POS gate was written
      against *adverbs*, which leave the reading undecided, and never considered a nominal; its
      two siblings already accept one (rule BC in this deprel for `obl`, rules AZ/CP in a caseless
      `obl` for both POS). Censused at 52 noun `advmod` rows, one a depictive and the rest
      adverbial quantifiers — rule CP's census shape, and the acceptance fires only where the LLM
      independently read the token as the complement. −1/+0.
    - **DY** (`_relative_adverb_oblique`): rule DD's POS gate read for the reason its docstring
      states. The gate separates a relative locative from a real preposition, and `onde` is a
      relative locative that Layer 2 tags four different ways (111 conjunction, 79 pronoun, 49
      adverb, 17 relative pronoun) under the same deprel — rule DT's lottery. The gate now also
      admits the locative **lemmas** ("lo cibo **onde** li pasca", "quel principio **ond'** elli
      indige"); lemma, not word, separates `onde` from `onda`. Censused at 32, of which 28 were
      already adverbs; −2/+0.
    - **DZ** (`_conjunct_named_by_phrase_head`): rule AI's NP-head equivalence read **through**
      rule C's coordination collapse ("Or voglion … **chi rincalzi** / li moderni pastori e **chi
      li meni** … e **chi di rietro li alzi**"). The first of the three free relatives is already
      accepted by rule AI, whose test is same-line; the other two are the same convention one
      `conj` edge further out, and nothing reached them because the collapse has already run. The
      "which normalization has run on the citation" question (rules CD, CI, DT) in its composed
      form. Censused at 85; −2/+0.
    - **EA** (`speech_act_nominal`): the elided speech verb Layer 4 records as a `parataxis` on a
      bare pronoun ("Ed **ella**: «…»"). `derive_unit` promotes the pronoun to a predicate and
      nothing attaches, so the whole tuple is rule M's ∅ subject — which asserts a dropped subject
      and nothing about any other slot, rule DA's boundary one step in. Deliberately **not**
      widened to that tuple shape in general (720 corpus-wide, and rule CS measured opening them
      at +180): gated to Layer 4's own record of the ellipsis, a `parataxis` on a non-verb,
      censused at **4**. Kept as an acceptance rather than as a refusal to mint, because refusing
      would turn the LLM's *correct* reading into an `extra_tuple`. −1/+0.

30. **Phase 6 — rules EB-EF, from the Paradiso 26-33 read** (2026-08-17). **Five** rules — four
    acceptances and one in `derive_unit` — each censused, measured alone by violation diff and
    mutation-checked; together with 16 Layer-4 rows, 6 Layer-2 rows, 4 Layer-3 spans and 2
    case-annex rows, **234 → 213 soft**, no model calls. **The read series now covers all 100
    cantos.** Two findings: a gate that names a part of speech *or a deprel* is a claim about a
    column, and `come` fills eight deprels and four tags with one reading (rule EB); and **a
    position an earlier batch assigned to the prompt can be checker silence** — paradiso 23:10 was
    written up as `_CONV_ADJUNCT` work three batches ago and rule EB takes it for free. Full
    evidence in [`CORRECTIONS.md`](CORRECTIONS.md).
    - **EB** (`_comparative_come_adjunct`): rule AR's marker gate names the **word**. `come`/`com`
      is written 812 times under eight deprels (543 `mark`, 145 `case`, 103 `advmod`, …) and four
      Layer-2 tags, and the gate admitted only `mark` + conjunction (441 rows) — while "**come**
      sole in viso che più trema, **così** … scema" (paradiso 30:25) is rule AR's own correlative
      shape written `advmod` + adverb. Rule DY's finding with the edge column added. −3/+0.
    - **EC** (`_comparative_come_adjunct`): rule AR's no-correlative branch, opened by rule BA's
      evidence. Two derived subjects mean two clauses collapsed onto one head, and rule CW drops
      the elided one's remnants *by position* — which fails where Dante puts the second term's
      subject last ("**come** a l'ultimo suo ciascuno artista", paradiso 30:33). The marker is the
      gap boundary the tree does state. Censused at 13 against 598; −1/+0.
    - **ED** (`_comparison_clause_hosts`): rule AR's `extra_arg` leg from the matrix side — the
      verbless comparison Layer 4 heads on the `come` itself ("**sì come** infino ad esso,
      succedono Ebree", paradiso 32:16), whose adjunct the LLM lists on the matrix predicate.
      Routed through rule X's mechanism, so it inherits the role-must-match gate. Censused at 14;
      −1/+0.
    - **EE** (`_prep_stack_nominal`): rule BV's opening-word leg. The 2026-08-14 normalization
      makes the cluster's first word a `case` row its `fixed` members hang on, and rule BV's
      reading — a preposition's own words are not arguments — covers it without a further claim
      ("Poscia che **'ncontro a** la vita presente", paradiso 28:1). A plain `case` word with no
      `fixed` child stays excluded. Censused at 167 structural, 1 cited; −2/+0, because the shape
      reports twice.
    - **EF** (`derive_unit`): the `conj` shared-subject propagation stops at a **sibling** conjunct
      that has already supplied one ("Concreato fu **ordine** … **pura potenza** tenne … nel mezzo
      **strinse**", paradiso 29:31-35). Rule AT decides who inherits and rule DU where a
      subordinator cuts the chain; this is whether the chain head is still the nearest antecedent.
      A **refusal, not a re-assignment**: handing the conjunct the nearer subject was measured at
      **+8/−2** and rejected, while stopping lets step 4's pro-drop ∅ and the authority model
      decide — rule DA's boundary from the other side. Censused at 23 of 3658; −3/+1.

31. **Phase 6 — rule EG, from the sixth `--fix` round's own output** (2026-08-18). The first soft
    check in this layer that compares the artifact with **itself**: one token filling two roles of
    one predicate. Every rule from V to EF compares the reading with `derive_unit`, and that
    comparison is blind here — when one of the two rows matches the derivation, the other is
    constrained by nothing — so **52 of the 56 positions were on lines `--check` said nothing
    about**, which is why 21 read batches walked past them. The licensed exception is rule AL's
    fused clitic (7), granted by calling **that rule's own gate** rather than by copying its
    condition. **174 → 224 (+50)**: the count going up is the rule's purpose, the trade rule AM
    records. Landed with three companions: a **splice guard** in `_apply_missing_arg` (the class
    appends a row and never looked at the rows already there — which is how the round wrote
    paradiso 1:81, `alcun` as both `subj` and `obj` of `fece`); the **`arg_slot` merge**, which asks
    a same-slot `missing_arg`/`extra_arg` pair as one question rather than two that cannot see each
    other; and **`_CONV_DATIVE` rewritten**, its old "cite it as `iobj`" being unfollowable in the
    class it hangs on. `pytest` 494 → **511**. Full write-up in [`CORRECTIONS.md`](CORRECTIONS.md).
32. **Phase 6 — rule EH, from the seventh `--fix` round's one refusal** (2026-08-18). Purgatorio
    2:40's `sen` = `si`+`ne` is written as `obl:si` **and** `obl:ne` of `venne` — rule AL/CM's
    licensed fused clitic — and rule EG reported it anyway, because `_case_supports_role` sent every
    `obl:<marker>` role to the `ablative` slot whatever the marker said, so both obliques collided on
    one slot and rule CM's "the two supporting slot sets must differ" rejected the pair. Layer 2's
    ...
    precedent — one gate refusing a case it accepts in every other notation. **161 → 160** (−1/+0),
    `pytest` 521 → **527**. Found by the model answering `both`, the single refusal among 48
    `dual_role` calls. Full write-up in [`CORRECTIONS.md`](CORRECTIONS.md).
33. **Phase 7 — rule EI, the floating quantifier** (2026-08-18). "e **tutta quanta** … **faceva**
    dir" (purgatorio 10:58), "e **son** … **tutti quanti**" (inferno 31:32): Layer 4 hangs the
    quantifier on the noun as an adnominal, Layer 3 enumerates `[tutta quanta]` as a noun phrase of
    its **own**, and `SYSTEM_PROMPT` tells the model to cite a phrase's head — so the reading cites
    the quantifier and `derive_unit` cites the noun. Rule AI accepts this convention already, but
    only inside a *single* NP span, which a floating quantifier by definition never occupies: this
    is the case rule AI cannot reach. Gated on a Layer-4 adnominal edge read **through rule C's
    coordination collapse**, on a closed quantifier lemma list, and on Layer 2 calling the token an
    adjective/numeral/pronoun rather than a noun. Censused at **53**, measured **−4/+0**,
    `pytest` 534 → **542**. Found by reading `arg_slot`'s eight refusals — the class the model
    refused at 100% in two consecutive `--fix` rounds. The same batch retagged purgatorio 9:97 in
    Layer 4 at Layer-5 net zero. Full write-up in [`CORRECTIONS.md`](CORRECTIONS.md).
...
- **Phase 6 Restructured `--fix`, Rules AG–EH**: Reduced soft violations from **2,084 → 1,091** (first user-run pass: 2011 → 1452; Rule AG: 1452 → 1409; second user-run pass: 1409 → 1247; Rules AH–AL and the Inferno 7–10 read: 1247 → 1091), then **1094** after the Layer-4 agreement close and prep-stack normalization (net zero by design), then **963** with the third user-run pass (1094 → 963, 2026-08-15), then **888** with rules AM–AT and the Inferno 11–15 read (963 → 888, 2026-08-15 — eight rules, four of them in `derive_unit` itself, plus 16 Layer-4 and 2 Layer-2 rows), then **834** with rules AU–AY and the Inferno 16–20 read (888 → 834, 2026-08-15 — five rules, three of them mirror legs of existing ones, plus 25 Layer-4 and 1 Layer-2 rows), then **691** with rules AZ–BI and the Inferno 21–25 read (834 → 691, 2026-08-15 — nine rules, plus 20 Layer-4 and 5 Layer-2 rows), then **650** with rules BJ–BN and the Inferno 26–30 read (691 → 650, 2026-08-15 — five rules plus three legs added to existing ones, and 10 Layer-4 and 1 Layer-2 rows), then **541** with the fourth user-run pass (650 → 541, 2026-08-16), then **506** with rules BO–BV and the Inferno 31–34 read (541 → 506, 2026-08-16 — eight rules, plus 15 Layer-4 rows, 2 Layer-2 rows, 1 Layer-3 span and 1 case-annex row), then **481** with rules BW–BZ and the Purgatorio 1–5 read (506 → 481, 2026-08-16 — four rules, plus 2 Layer-4 rows and 1 case-annex row), then **448** with rules CA–CJ and the Purgatorio 6–10 read (481 → 448, 2026-08-16 — ten rules, eight of them about coordination or about a predicate the derivation declines to mint, plus 1 Layer-4 row), then **427** with rules CK–CO and the Purgatorio 11–15 read (448 → 427, 2026-08-16 — five rules, the `dep.subject_agreement` coordinated-subject refinement the Inferno 21–25 batch had deferred, plus 11 Layer-4 rows, 8 Layer-2 rows, 1 Layer-3 span and 1 case-annex row), then **409** with rules CP–CT and the Purgatorio 16–20 read (427 → 409, 2026-08-16 — four Layer-5 rules and the `dep.subject_agreement` person refinement, plus 17 Layer-4 rows, 2 Layer-2 rows, 1 Layer-3 span and 2 case-annex rows), then **388** with rules CU–CY and the Purgatorio 21–25 read (409 → 388, 2026-08-16 — four Layer-5 rules and the `dep.subject_agreement` ordering refinement, plus 27 Layer-4 rows and 1 Layer-2 row), then **358** with rules CZ–DD and the Purgatorio 26–30 read (388 → 358, 2026-08-17 — five Layer-5 rules, one of them in `derive_unit`, plus 12 Layer-4 rows), then **351** with rules DE–DF and the Purgatorio 31–33 read (358 → 351, 2026-08-17 — two Layer-5 rules, plus 4 Layer-4 rows, 1 Layer-2 row, 2 Layer-3 spans and 2 case-annex rows), then **298** with the fifth user-run pass (351 → 298, 2026-08-17 — run with no prompt change and no model change, so it tested no hypothesis; what it measured is that the read series' rules and the LLM's repairs take largely disjoint residue), then **288** with rules DG–DJ and the Paradiso 1–5 read (298 → 288, 2026-08-17 — four Layer-5 rules, plus 6 Layer-4 rows, 1 Layer-2 row, 1 Layer-3 span and 2 case-annex rows), then **261** with rules DK–DR and the Paradiso 6–10 read (288 → 261, 2026-08-17 — eight Layer-5 rules, plus 10 Layer-4 rows, 2 Layer-2 rows and 2 case-annex rows), then **245** with rules DS–DW and the Paradiso 11–20 read (261 → 245, 2026-08-17 — five Layer-5 rules, plus 9 Layer-4 rows and 1 Layer-2 row, and the sixth round's queued prompt clause written), then **234** with rules DX–EA and the Paradiso 21–25 read (245 → 234, 2026-08-17 — four Layer-5 rules, plus 10 Layer-4 rows, 1 Layer-2 row and 1 Layer-3 span), then **213** with rules EB–EF and the Paradiso 26–33 read (234 → 213, 2026-08-17 — five rules, one of them in `derive_unit`, plus 16 Layer-4 rows, 6 Layer-2 rows, 4 Layer-3 spans and 2 case-annex rows; **the read series now covers all 100 cantos**), then **174** with the sixth user-run pass (213 → 174, 2026-08-18 — the round the read series was written to enable, so its per-class numbers measure the prompt alone: `_CONV_ADJUNCT` positive at −52.6% on the prepositional obliques its prose names, `_CONV_DATIVE` negative; and its one new position exposed a class of error no check looks for, one token in two roles of one predicate), then **224** with **rule EG** (174 → 224, 2026-08-18 — +50 **by design**: the first check in this layer that reads the artifact *against itself*, one token in two roles of one predicate, censused at 56 with 7 licensed by rule AL's fused clitic and 52 of the 56 previously on lines `--check` said nothing about; plus a splice guard so no `--fix` round writes another, the `arg_slot` merge that asks a same-slot pair once instead of twice, and `_CONV_DATIVE` rewritten), then **161** with the seventh user-run pass (224 → 161, 2026-08-18 — the first round run with `--log`, so the first with per-class call counts: `dual_role` −82.0% carried 40 of the 63 while the divergence residue moved −12.6%, and per call rule EG's artifact-internal question ran at 0.833 against 0.081 for every other class; 30% of the round's calls ended in the model refusing, and the refusal split landed the same day to count them), then **160** with **rule EH** (161 → 160, 2026-08-18 — the fused clitic named after its own words, the seventh round's one concrete checker finding, found by reading a refusal). Phase 6 is closed; the full record is in [`PHASE6.md`](PHASE6.md).
- **Phase 7, rounds and refusal-census reads**: **160 → 154** with the eighth user-run pass (2026-08-18 — the first run `--no-whole`, and the round that closed the question of whether more rounds pay: 142 calls for 6 positions, 43.7% of them the model refusing, so the residue is checker-side work from here; its per-class refusal table reproduced the seventh round's almost exactly, which turns the census into a settled reading list — see [`PLAN.md`](PLAN.md) §P1), then **150** with **rule EI** and the first refusal-census read (154 → 150, 2026-08-18 — `arg_slot`'s 8 refusals read one by one: two were the floating-quantifier shape rule AI's single-span test cannot reach, one was a Layer-4 mis-parse retagged at Layer-5 net zero, one was censused at 2 and dropped, and four are genuine reading disagreement; see [`PLAN.md`](PLAN.md) §P2). Phase 7's plan is in [`PLAN.md`](PLAN.md). See also [`CORRECTIONS.md`](CORRECTIONS.md).

## Next steps

The read series is closed (all 100 cantos read position by position) and the open route is the
**seventh `--fix` round**, which is the user's to run — base 224, with three separable candidates on
its scale: rule EG's brand-new `dual_role` class (50 positions that have never been asked), the
`arg_slot` merge (16) and the rewritten `_CONV_DATIVE` (11). The standing assistant-side route after
it is the **subject slot**, 62 of the 174 divergence positions. See [`PLAN.md`](PLAN.md)'s *The
Seventh `--fix` Round* and *Active & Open Routes*.
