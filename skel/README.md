...
**Status: built for all 100 cantos, checker refined through Phase 5 (5,919 → 2,084 soft) and Phase 6 (rules AG–EH plus seven `--fix` rounds: 160 soft), and now under Phase 7, whose eighth/ninth rounds and rule EI took it to 140 soft — 137 divergence positions plus the 3 artifact-internal contradictions rule EG reports. `--fix` operates as a three-stage driver (deterministic auto-repair, POS-keyed micro-prompts, and fallback regeneration — the last measured and switched off permanently from round 8 on).**

`make -C skel check`: **0 hard, 140 soft** violations across all 100 cantos (the divergence residue is 137, down from 17,438 at the first full-corpus measurement; rule EG reports 3 artifact-internal contradictions — see [`CORRECTIONS.md`](CORRECTIONS.md)). Full historical measurement tables, per-phase progressions and empirical findings are documented in [`PHASE5.md`](PHASE5.md) (Phase 5) and [`PHASE6.md`](PHASE6.md) (Phase 6 — the seven rounds, the nineteen-batch read of all 100 cantos, and its transferable findings). For current Phase 7 operating principles, active routes, the ninth round's write-up (§P3) and driver architecture, see [`PLAN.md`](PLAN.md) and [`CORRECTIONS.md`](CORRECTIONS.md).
