...
`morph/CORRECTIONS.md`). One case did **not** qualify and was left unflagged: inferno 19:73-74
(`son li altri tratti / che precedetter me simoneggiando`) attaches the relative clause to
`tratti` (a passive participle, "[they are] dragged") rather than to `altri` ("the others"), its
more plausible antecedent — a genuine likely mis-attachment, not a substantivization, and the
model reproduced it identically across `--fix` regenerations. Left as the corpus's one accepted
Layer 4 soft violation.
...
`dep --check`: **0 hard, 1 soft** (down from 132; the `tratti` case above). `morph --check` and
`np --check` remain 0 hard / 0 soft — the new flag doesn't intersect `_needs_np`'s exemption set,
so Layer 3 is unaffected.
