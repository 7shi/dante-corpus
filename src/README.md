...
## `ref/` — reference examples (moved out of this repository)

**These files are no longer here.** `src/ref/` moved to the `dante-analyze`
project (commit `859a5d9`, "Remove files moved to dante-analyze"), which is where
the interpretation-side work they belong to lives. The description below is kept
as a record of what they were, so the rounds that cite them stay readable.

`ref/inferno-ja.md`, `ref/purgatorio-ja.md`, and `ref/paradiso-ja.md` were
**Japanese** reference breakdowns generated by GitHub Copilot (GPT-5.4),
following the policy in `ref/AGENTS.md`. They predate `scene.py` and were kept as
worked examples of the intended structure and granularity; their table headers
(`行` / `場面名` / `内容`) and summaries are in Japanese.

`ref/inferno-01.toml` was a hand-written **context-lock** sample for Inferno
Canto 1 — a per-scene skeleton that fixes identity only (who speaks, where, who
"this light" is) as translation pre-processing, with a `basis` quote per entry
for verification against the source. It was kept to compare against a
model-generated version later; the design and the error patterns it guards
against went with it.
